...
n=$[`cat data/train/feats.scp | wc -l` - 4000]
...
#dir=exp_110h/train_phn_l${lstm_layer_num}_c${lstm_cell_dim}_u${num_utts}
...
$cuda_cmd $dir/log/train_ctc_parallel.log \
  steps/train_ctc_parallel.sh --add-deltas true --num-sequence $num_utts --frame-num-limit 20000 \
...
for lm_suffix in sw1_tg sw1_fsh_tgpr; do
  steps/decode_ctc_lat.sh --cmd "$decode_cmd" --nj 20 --beam 17.0 --lattice_beam 8.0 --max-active 5000 --acwt 0.6 \
    data/lang_phn_${lm_suffix} data/eval2000 $dir/decode_eval2000_${lm_suffix} &
done
wait
exit
...
# Train the network with CTC. Refer to the script for details about the arguments
$cuda_cmd $dir/log/train_ctc_parallel.log steps/train_ctc_parallel.sh --add-deltas true --num-sequence 10 --frame-num-limit 25000 \
